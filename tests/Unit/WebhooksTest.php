...
final class WebhooksTest extends TestCase
{
    public function test_signature_uses_sha256_hmac_with_prefix(): void
    {
        ...
        $this->assertSame($expected, \zignites_chat_webhook_signature($body, $secret));
    }

    public function test_signature_returns_empty_for_empty_body_or_secret(): void
    {
        $this->assertSame('', \zignites_chat_webhook_signature('', 'secret'));
        $this->assertSame('', \zignites_chat_webhook_signature('body', ''));
    }

    public function test_signature_is_stable_for_same_inputs(): void
    {
        $a = \zignites_chat_webhook_signature('payload', 'k');
        $b = \zignites_chat_webhook_signature('payload', 'k');
        $this->assertSame($a, $b);
    }

    public function test_signature_changes_with_body(): void
    {
        $a = \zignites_chat_webhook_signature('payload-a', 'k');
        $b = \zignites_chat_webhook_signature('payload-b', 'k');
        $this->assertNotSame($a, $b);
    }

    public function test_signature_changes_with_secret(): void
    {
        $a = \zignites_chat_webhook_signature('payload', 'k1');
        $b = \zignites_chat_webhook_signature('payload', 'k2');
        $this->assertNotSame($a, $b);
    }

    public function test_event_keys_includes_known_lifecycle_events(): void
    {
        $keys = array_keys(\zignites_chat_webhook_event_keys());
    ...
    {
        $env = \zignites_chat_webhook_payload('message.sent', ['order_id' => 42]);
    ...
    {
        $env = \zignites_chat_webhook_payload('message.sent', null);
        $this->assertSame([], $env['data']);
    }

    public function test_sanitize_drops_invalid_url(): void
    {
        $this->assertNull(\zignites_chat_sanitize_webhook(['url' => '', 'events' => ['message.sent']]));
        $this->assertNull(\zignites_chat_sanitize_webhook(['url' => 'not-a-url', 'events' => ['message.sent']]));
        $this->assertNull(\zignites_chat_sanitize_webhook(['url' => 'ftp://example.com', 'events' => ['message.sent']]));
    }

    public function test_sanitize_drops_when_no_valid_events(): void
    {
        $this->assertNull(\zignites_chat_sanitize_webhook(['url' => 'https://example.com', 'events' => []]));
        $this->assertNull(\zignites_chat_sanitize_webhook(['url' => 'https://example.com', 'events' => ['nonsense.event']]));
    }

    public function test_sanitize_dedupes_and_filters_events(): void
    {
        $clean = \zignites_chat_sanitize_webhook([
            'url'    => 'https://example.com/zignitesChat',
        ...
        $this->assertSame('https://example.com/zignitesChat', $clean['url']);
    ...
    public function test_filter_webhooks_for_event_returns_only_active_subscribed(): void
    {
        ...
        $matches = \zignites_chat_filter_webhooks_for_event($webhooks, 'message.sent');
    ...
    {
        $this->assertSame([], \zignites_chat_filter_webhooks_for_event([], 'message.sent'));
        $this->assertSame([], \zignites_chat_filter_webhooks_for_event([['id' => 'a', 'active' => true, 'events' => ['x']]], ''));
    }
}
