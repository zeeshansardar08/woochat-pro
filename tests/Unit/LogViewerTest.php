...
final class LogViewerTest extends TestCase
{
    ...
    {
        $this->tmp_file = tempnam(sys_get_temp_dir(), 'zignites-chat-log-test-');
    ...
    {
        $parsed = \zignites_chat_log_parse_line('[Zignites Chat] Sent message to ••••1234');
        $this->assertSame('Zignites Chat', $parsed['tag']);
        $this->assertSame('Sent message to ••••1234', $parsed['message']);
        $this->assertSame('[Zignites Chat] Sent message to ••••1234', $parsed['raw']);
    }

    public function test_parse_line_preserves_subtag(): void
    {
        $parsed = \zignites_chat_log_parse_line('[Zignites Chat - Cart Recovery] Attempt evt_xyz to ••••1234: Hi!');
        $this->assertSame('Zignites Chat - Cart Recovery', $parsed['tag']);
        $this->assertSame('Attempt evt_xyz to ••••1234: Hi!', $parsed['message']);
    }

    public function test_parse_line_handles_unprefixed_lines(): void
    {
        $parsed = \zignites_chat_log_parse_line('Stray line without prefix');
    ...
    {
        $parsed = \zignites_chat_log_parse_line('');
        $this->assertSame('', $parsed['tag']);
    ...
    public function test_tail_returns_only_last_n_when_file_is_larger(): void
    {
        ...
        $lines = \zignites_chat_log_tail_lines($file, 5);
    ...
    {
        $lines = [
            '[Zignites Chat] Sent OK',
            '[Zignites Chat] Send failed: timeout',
            '[Zignites Chat - Cart Recovery] Sent reminder',
        ];
        $filtered = \zignites_chat_log_filter_lines($lines, 'sent');
    ...
    public function test_filter_lines_applies_tag_filter(): void
    {
        $lines = [
            '[Zignites Chat] Sent OK',
            '[Zignites Chat - Cart Recovery] Sent reminder',
            '[Zignites Chat - MANUAL] Sent manual',
        ];
        $filtered = \zignites_chat_log_filter_lines($lines, '', 'Zignites Chat - Cart Recovery');

        $this->assertCount(1, $filtered);
        $this->assertSame('Zignites Chat - Cart Recovery', $filtered[0]['tag']);
    }
    ...
    public function test_filter_lines_combines_keyword_and_tag(): void
    {
        $lines = [
            '[Zignites Chat] Sent OK',
            '[Zignites Chat - Cart Recovery] Sent reminder',
            '[Zignites Chat - Cart Recovery] Skipped opted-out number',
        ];
        $filtered = \zignites_chat_log_filter_lines($lines, 'opted', 'Zignites Chat - Cart Recovery');
    ...
    {
        $lines = ['[Zignites Chat] One', '[Zignites Chat] Two'];
        $filtered = \zignites_chat_log_filter_lines($lines, '', '');
    ...
    {
        $entries = [
            ['tag' => 'Zignites Chat - Cart Recovery'],
            ['tag' => 'Zignites Chat'],
            ['tag' => 'Zignites Chat - Cart Recovery'],
            ['tag' => ''],
            ['tag' => 'Zignites Chat - MANUAL'],
        ];
        $tags = \zignites_chat_log_tags_present($entries);

        $this->assertSame(
            ['Zignites Chat', 'Zignites Chat - Cart Recovery', 'Zignites Chat - MANUAL'],
            $tags
        );
    }
    ...
    {
        $this->assertSame([], \zignites_chat_log_tags_present([]));
    }
}
