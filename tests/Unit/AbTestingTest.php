...
final class AbTestingTest extends TestCase
{
    ...
    {
        $first  = \zignites_chat_ab_pick_variant('order', '12345');
        $second = \zignites_chat_ab_pick_variant('order', '12345');
        $third  = \zignites_chat_ab_pick_variant('order', '12345');
    ...
    {
        for ($i = 0; $i < 50; $i++) {
            $v = \zignites_chat_ab_pick_variant('cart_recovery', 'phone-' . $i);
    ...
    public function test_pick_variant_distributes_roughly_evenly(): void
    {
        ...
        for ($i = 0; $i < 1000; $i++) {
            $v = \zignites_chat_ab_pick_variant('order', 'order-' . $i);
            if ($v === 'a') { $a++; } else { $b++; }
        }
    ...
    public function test_pick_variant_is_kind_scoped(): void
    {
        ...
        for ($i = 0; $i < 100; $i++) {
            $key = 'k' . $i;
            \zignites_chat_ab_pick_variant('order', $key) === 'a' ? $a_orders++ : $b_orders++;
            \zignites_chat_ab_pick_variant('cart_recovery', $key) === 'a' ? $a_carts++ : $b_carts++;
        }
    ...
    {
        $this->assertSame('a', \zignites_chat_ab_pick_variant('', 'key'));
        $this->assertSame('a', \zignites_chat_ab_pick_variant('order', ''));
    }

    public function test_get_template_returns_a_when_disabled(): void
    {
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_template'] = 'Variant A body';
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_template_b'] = 'Variant B body';
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_ab_enabled'] = 'no';

        $picked = \zignites_chat_ab_get_template('order', '42');
    ...
    {
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_template'] = 'Variant A body';
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_template_b'] = '   '; // whitespace only
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_ab_enabled'] = 'yes';
    ...
    {
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_template'] = 'A body';
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_template_b'] = 'B body';
        $GLOBALS['zignites_chat_test_options']['zignites_chat_order_message_ab_enabled'] = 'yes';

        // Find a key that lands on B so we can assert the lookup wired up.
        $b_key = null;
        for ($i = 0; $i < 50; $i++) {
            if (\zignites_chat_ab_pick_variant('order', 'k' . $i) === 'b') {
                $b_key = 'k' . $i;
                break;
            }
        }
        ...
        $picked = \zignites_chat_ab_get_template('order', $b_key);
    ...
    {
        $picked = \zignites_chat_ab_get_template('not-a-kind', 'k1');
        $this->assertSame('a', $picked['variant']);
    ...
    {
        $this->assertSame(['a' => [], 'b' => []], \zignites_chat_ab_partition_events_by_variant([]));
        $this->assertSame(['a' => [], 'b' => []], \zignites_chat_ab_partition_events_by_variant(null));
    }
